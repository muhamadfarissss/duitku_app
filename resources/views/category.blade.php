...
<x-app-layout>
    <div class="px-10 py-12 max-w-7xl mx-auto">
        ...
        <div class="flex space-x-3 mb-8">
            <button class="px-6 py-2.5 bg-white border border-[#0061cc] text-[#0061cc] font-semibold rounded-xl shadow-sm flex items-center gap-2">
                <span class="text-[#0061cc]">↙</span> Pengeluaran (6)
            </button>
            <button class="px-6 py-2.5 bg-white border border-gray-200 text-gray-500 font-medium rounded-xl hover:bg-gray-50 transition-colors flex items-center gap-2">
                <span class="text-gray-400">↗</span> Pemasukan (3)
            </button>
        </div>
        ...
        <div class="bg-white w-full max-w-md rounded-3xl p-8 shadow-2xl relative">
            ...
            <form id="formKategori" onsubmit="saveCategoryWithBudget(event)" class="space-y-5">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-7 gap-2 max-h-32 overflow-y-auto p-1" id="iconContainer">
                        </div>
                ...
                <div>
                    ...
                    <input id="catBudget" type="number" placeholder="1000000" class="w-full border border-gray-300 rounded-xl py-3 px-4 text-sm outline-none focus:ring-2 focus:ring-[#0061cc]">
                </div>
    ...
    <script>
        ...
        // ==== LOGIKA EMOJI PICKER DIPERBARUI DENGAN BANYAK IKON ====
        const emojis = [
            // Makanan & Minuman
            '🍔', '☕', '🍱', '🍩', '🍕', '🍰',
            // Transportasi
        ...
        emojis.forEach((emoji, index) => {
            ...
            iconContainer.innerHTML += `
                <button type="button" onclick="selectIcon('${emoji}', this)" class="icon-btn w-10 h-10 flex items-center justify-center text-xl rounded-xl transition-all ${isSelected}">
                    ${emoji}
                </button>
            `;
        });
        ...
        function loadCategoriesAndBudgets() {
            ...
            .then(([cats, buds, summ]) => {
                ...
                (cats.data || []).forEach(cat => {
                    ...
                    // Gunakan icon dari database, kalau kosong pakai default 🍔
                    ...
                    grid.innerHTML += `
                        <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow">
                            ...
                            <div class="mt-6 pt-4 border-t border-gray-50 flex justify-between items-end">
                                ...
                                <div class="text-[10px] text-gray-400 font-medium">dari ${limit > 0 ? formatRupiah(limit) : 'Tanpa Limit'}</div>
                            </div>
                        </div>
                    `;
                });
                ...
                if(maxWarning.percent >= 75) {
                    insightText.innerText = `Pengeluaran kategori ${maxWarning.cat} sudah mencapai ${maxWarning.percent}% dari budget. Hindari pembelian barang sekunder hingga akhir bulan agar kondisi finansial tetap stabil.`;
                } else {
        ...
        function saveCategoryWithBudget(e) {
            ...
            const payload = {
                ...
                // type: document.getElementById('catType').value // Jika nanti backend-mu butuh field 'type'
            };
